...
Two types of tests:
  1. Round-trip: push a point through both directions and verify it comes
     back to within floating-point precision.

  2. Cross-check against pyproj: verify our result agrees with the industry-
     standard library to within a few centimetres (pyproj's CRS path does
     not apply the same ITRF2020->GDA2020 Helmert, so mm-level differences
     are expected).
"""
...
# Flinders Peak (Table 5.1 / Table C-1, GDA2020 Technical Manual v1.8)
...
class Test_MGA2020_to_WGS84_RoundTrip:
    """
    Round-trip test: MGA2020 -> WGS84 -> MGA2020.
    """

    def test_easting_northing_round_trip(self):
        lat, lon = mga2020_to_wgs84(EASTING, NORTHING, epoch=2020.0, zone=55)
        E_rt, N_rt, zone = wgs84_to_mga2020(lat, lon, epoch=2020.0, zone=55)
        # Published E/N are rounded to mm; allow that level of closure
        assert E_rt == pytest.approx(EASTING, abs=1e-3)
        assert N_rt == pytest.approx(NORTHING, abs=1e-3)
        assert zone == 55


class Test_WGS84_to_MGA2020_Pyproj:
    """
    Cross-check our forward result against pyproj.
    """
...
class Test_MGA2020_to_WGS84_Pyproj:
    """
    Cross-check our inverse result against pyproj.
    """

    def test_lat_lon_pyproj(self):
        ...
        lat_ours, lon_ours = mga2020_to_wgs84(EASTING, NORTHING, epoch=2020.0, zone=55)

        # ~1e-7 deg ≈ 1 cm on the ground; matches the 1 cm forward tolerance
        assert lat_ours == pytest.approx(lat_pyproj, abs=1e-7)
        assert lon_ours == pytest.approx(lon_pyproj, abs=1e-7)
